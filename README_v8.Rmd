...
gunzip ${RAREVARDIR}/data/GTEx/gencode.v26.annotation.gtf.gz
...
### Subset 1KG
Subset 1KG for African samples
```{bash subset_1KG, eval=FALSE, cache=TRUE}
skg=/scratch/groups/abattle4/jessica/1KG_vcf_biallelic
gen_data_dir=/scratch/groups/abattle4/jessica/ATG/Final/data
...
vcftools --vcf ${skg}/ALL.chr${j}.shapeit2_integrated_v1a.GRCh38.20181129_v42.phased.vcf \
  --keep ${gen_data_dir}/1kg_afr.keep \
...
1c on v6 data showing replication rates… very different values
```{bash}
...
Get list of all european subjects with WGS
```{bash}
# list of all African American individuals from GTEx v8 (some do not have WGS data)
cat ${RAREVARDIR}/archive/download/gtex8/GTEx_Analysis_2017-06-05_v8_Annotations_SubjectPhenotypesDS.txt | awk -F '\t' '{if ($5 == 2) print $1;}' \
> ${RAREVARDIR}/archive/reference/gtex_v8_individuals_EUR.txt

# list of all African American individuals from GTEx v8 with WGS data
awk 'NR==FNR { lines[$0]=1; next } $0 in lines' ${RAREVARDIR}/archive/reference/gtex_v8_wgs_individuals.txt ${RAREVARDIR}/archive/reference/gtex_v8_individuals_EUR.txt > ${RAREVARDIR}/archive/reference/gtex_v8_wgs_individuals_AFA.txt
```
