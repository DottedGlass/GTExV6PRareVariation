...
covs.dir = paste0(dir, '/data/GTEx/GTEx_Analysis_v8_eQTL_covariates/')

# Read in list of tissues with eQTL data
tissues = read.table(paste0(dir,'/reference/gtex_v8_tissues_all_normalized_samples.txt'), header = F, stringsAsFactors = F)[, 1]

# For each tissue, read in the RPKM and covariate files
# Subset and reorder columns of RPKM file to match covariate file
# Filter for genes with >= 10% of individuals with RPKM > 0.1 and read count > 6
...
for(i in 1:length(tissues)){
	...
	print(i)
	print(tissue)
	rpkm = as.data.frame(fread(paste0(peer.dir, tissue, '.tpm.txt'), header = T))
	...
	covariates = read.table(paste0(covs.dir, tissue, '.v8.covariates.txt'), header = T, stringsAsFactors = F, row.names = 1)
	colnames(covariates) = gsub('\\.', '-', colnames(covariates))

	rpkm = rpkm[, colnames(covariates)]
	reads = reads[, colnames(covariates)]
	indices_to_keep = rowSums(rpkm > rpkm_filt & reads > read_filt) >= ind_filt_per * ncol(rpkm)
	rpkm = rpkm[indices_to_keep, ]
	rpkm_out = scale(t(log2(rpkm + 2)))
	write.table(rpkm_out, paste0(peer.dir, tissue, '.tpm.log2.ztrans.txt'), quote = F, sep = '\t', row.names = T, col.names = T)
}
