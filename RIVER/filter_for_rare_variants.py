...
def filter_rare(gtex_df, gtex_pos_dict, oneKG_df, oneKG_dict, MAF_thres=0.01):
    ...
    for i in range(len(gtex_df)):
        ...
        allele_list = list(
            gtex_df.iloc[i, :][4:]
        )  # list causes NaN to be cast as float nan
        frq = {
...
#     return df_new
#
# def filter_rare_vectorized(gtex_df, gtex_pos_dict, oneKG_df, oneKG_dict, MAF_thres = 0.01):
#     """Filter for rare variants based on allele frequncy < 0.01 in both GTEx and 1KG
#     Returns gtex_rare_df
#     """
#
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Filter for rare variants based on allele frequency <0.01 in both GTEx and 1KG"
    )
    ...
    parser.add_argument(
        "outfile", type=str, help="output file for filtered list of rare variants"
    )
    ...
    gtex_rare_df = filter_rare(
        gtex_df, gtex_pos_dict, oneKG_df, oneKG_dict, MAF_thres=MAF_thres
    )
