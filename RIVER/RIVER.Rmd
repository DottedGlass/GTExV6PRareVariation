...
## get z-scores
euro_df <- euro_df %>% mutate(lowertail=total_expression_pvalue<0)

euro_df$Zscore = NA
euro_df$Zscore[euro_df$lowertail] = qnorm(abs(euro_df$total_expression_pvalue[euro_df$lowertail]) ,lower.tail=TRUE)
euro_df$Zscore[!euro_df$lowertail] = qnorm(abs(euro_df$total_expression_pvalue[!euro_df$lowertail]) ,lower.tail=FALSE)

## format columns
euro_df <- select(euro_df,-contains("pvalue"),-lowertail) # drop columns containing "pvalue"
...
euro_data_common <- "/scratch/groups/abattle4/victor/GTExV6PRareVariationData/data/RIVER/river_input_v8_european_all_05-21-2020.txt"
...
zscore_list_euro <- vector("numeric",nrow(euro_df))
for (i in seq(nrow(euro_df))) {
  subject <- as.character(euro_df[i,1])
  gene <- as.character(euro_df[i,2])
  zscore_list_euro[i] <- zscore_table[.(gene,subject)]$Zscore
}
...
legend(0.7,0.2,c("RIVER","GAM"), lty=c(1,1), lwd=c(2,2),
       col=c("dodgerblue","mediumpurple"), cex=1.2, 
       pt.cex=1.2, bty="n")
title(main=paste("AUC: RIVER = ", round(evaROC_euro_all$RIVER_auc,3), 
...
model <- appRIVER_euro_all(dataInput, pseudoc=50, 
...
save(model, file = outfile_model_euro_all)
```
...
legend(0.7,0.2,c("RIVER","GAM"), lty=c(1,1), lwd=c(2,2),
       col=c("dodgerblue","mediumpurple"), cex=1.2, 
       pt.cex=1.2, bty="n")
title(main=paste("AUC: RIVER = ", round(evaROC_euro_common$RIVER_auc,3), 
...
model <- appRIVER_euro_common(dataInput, pseudoc=50, 
                      theta_init=matrix(c(.99, .01, .3, .7), nrow=2), 
                      costs=c(100, 10, 1, .1, .01, 1e-3, 1e-4), 
                      verbose=TRUE)
...
save(model, file = outfile_model_euro_common)
```
...
legend(0.7,0.2,c("RIVER","GAM"), lty=c(1,1), lwd=c(2,2),
       col=c("dodgerblue","mediumpurple"), cex=1.2, 
       pt.cex=1.2, bty="n")
title(main=paste("AUC: RIVER = ", round(evaROC_afr_all$RIVER_auc,3), 
...
model <- appRIVER_afr_all(dataInput, pseudoc=50, 
                      theta_init=matrix(c(.99, .01, .3, .7), nrow=2), 
                      costs=c(100, 10, 1, .1, .01, 1e-3, 1e-4), 
                      verbose=TRUE)
...
save(model, file = outfile_model_afr_all)
```
...
legend(0.7,0.2,c("RIVER","GAM"), lty=c(1,1), lwd=c(2,2),
       col=c("dodgerblue","mediumpurple"), cex=1.2, 
       pt.cex=1.2, bty="n")
title(main=paste("AUC: RIVER = ", round(evaROC_afr_common$RIVER_auc,3), 
...
model <- appRIVER_afr_common(dataInput, pseudoc=50, 
                      theta_init=matrix(c(.99, .01, .3, .7), nrow=2), 
                      costs=c(100, 10, 1, .1, .01, 1e-3, 1e-4), 
                      verbose=TRUE)
...
save(model, file = outfile_model_afr_common)
```
